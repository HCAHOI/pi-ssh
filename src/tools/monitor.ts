	...
	pi.registerTool({
		...
		async execute(_id, params: {
			...
			if (params.action === "create") {
				...
				const m = await monitors.create(t, { source, pattern: params.pattern, repeat: params.repeat, name: params.name, notify, template: params.template, notifyWhen: params.notifyWhen });
			...
			if (params.action === "update") {
				if (params.pattern === undefined && params.repeat === undefined && params.name === undefined && params.notify === undefined && params.template === undefined && params.notifyWhen === undefined) {
					throw new Error("ssh_monitor update requires at least one of pattern, repeat, name, notify, template, notifyWhen");
				}
				const notify = params.notify === undefined ? undefined : parseNotifyPolicy(params.notify);
				const m = await monitors.update(params.id, { pattern: params.pattern, repeat: params.repeat, name: params.name, notify, template: params.template, notifyWhen: params.notifyWhen });
